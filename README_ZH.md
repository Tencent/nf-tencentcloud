...
[![Release](https://img.shields.io/badge/v1.0.3-v?label=realease)](https://github.com/Tencent/nf-tencentcloud/releases/tag/1.0.3)
...
- 您也可以在配置文件中指定插件从而进行安装。
  ```groovy
  plugins {
      id 'nf-tencentcloud@1.0.3'
  }
  ```
- 也可以使用 -plugins 命令行参数来指定工具。
  ```bash
  nextflow run <pipeline> -plugins nf-tencentcloud@1.0.3
  ```
