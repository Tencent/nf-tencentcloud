...
[![Release](https://img.shields.io/badge/v2.0.1-v?label=realease)](https://github.com/Tencent/nf-tencentcloud/releases/tag/2.0.1)
...
- 您也可以在配置文件中指定插件从而进行安装。
  ```groovy
  plugins {
      id 'nf-tencentcloud@2.0.1'
  }
  ```
- 也可以使用 -plugins 命令行参数来指定工具。
  ```bash
  nextflow run <pipeline> -plugins nf-tencentcloud@2.0.1
  ```
